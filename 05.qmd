...
# Set axis ranges and ticks (updated to match other figures)
plt.xlim(50, 150)
plt.ylim(25000, 275000)
plt.xticks(range(50, 175, 25))  # 50, 75, 100, 125, 150
plt.yticks(range(25000, 300000, 25000))  # 25k, 50k, 75k, ..., 275k
...
#| fig-cap: "Equilibrium in the Software Engineering Firm"

import matplotlib.pyplot as plt
import numpy as np

# Data points from the example - these are the equilibrium employment levels
years = ['2023', '2024']
wages = [100000, 110000]  # Wages paid to workers
employees = [100, 120]    # Equilibrium employment levels
...
# Marginal Factor Cost curve - has twice the slope of the supply curve for linear supply
# This is a standard result in monopsony theory
slope_mfc = 2 * slope_supply
intercept_mfc = intercept_supply  # Same intercept as supply curve
y_mfc = slope_mfc * x_line + intercept_mfc

# Plot the supply curve and MFC curve
plt.plot(x_line, y_supply, color='#2E86AB', linewidth=2, label='Labor Supply Curve')
plt.plot(x_line, y_mfc, color='#DC3545', linewidth=2, label='Marginal Factor Cost (MFC)')

# For monopsony equilibrium, MRP intersects MFC at the equilibrium employment levels
# The wage is read off the supply curve at those employment levels
# So MRP curves must pass through the MFC values at the equilibrium employment levels

# Calculate MFC values at equilibrium employment levels
mfc_2023 = slope_mfc * employees[0] + intercept_mfc
mfc_2024 = slope_mfc * employees[1] + intercept_mfc

# Choose a slope for MRP curves (negative since they're downward sloping)
mrp_slope = -1200  # Steeper than before to ensure proper intersections

# 2023 MRP curve: passes through (100, mfc_2023)
mrp_2023_intercept = mfc_2023 - mrp_slope * employees[0]
y_mrp_2023 = mrp_slope * x_line + mrp_2023_intercept

# 2024 MRP curve: passes through (120, mfc_2024) - this curve is shifted up
mrp_2024_intercept = mfc_2024 - mrp_slope * employees[1]
...
# Plot the MRP-MFC intersection points
mfc_values = [mfc_2023, mfc_2024]
plt.scatter(employees, mfc_values, s=80, color='#8B0000', alpha=0.8, edgecolors='black', linewidth=1, zorder=5, marker='s')
...
for i in range(len(employees)):
    plt.plot([employees[i], employees[i]], [wages[i], mfc_values[i]], 
             color='gray', linestyle=':', linewidth=1, alpha=0.7)

plt.xlabel('Number of Employees')
plt.ylabel('Annual Wage ($)')
plt.title('Equilibrium in the Software Engineering Firm')
plt.grid(True, alpha=0.3)
plt.legend(loc='lower left')

# Set axis ranges and ticks - updated to 275k range
plt.xlim(50, 150)
plt.ylim(25000, 275000)
plt.xticks(range(50, 175, 25))  # 50, 75, 100, 125, 150
plt.yticks(range(25000, 300000, 25000))  # 25k, 50k, 75k, ..., 275k
...
#| fig-cap: "Equilibrium in the Software Engineering Firm with Flat Labor Supply Curves"
...
# Create two relatively flat labor supply curves
supply_slope = 200  # Relatively flat - $200 increase per additional employee
...
# 2024 Labor supply curve: shifted up, passes through (120, 110000)
...
# MFC curves have same intercepts as their corresponding supply curves
y_mfc_2023 = mfc_slope * x_line + supply_2023_intercept
y_mfc_2024 = mfc_slope * x_line + supply_2024_intercept

# Calculate MFC values at the equilibrium employment levels
mfc_2023_eq = mfc_slope * employees[0] + supply_2023_intercept  # MFC at L=100
mfc_2024_eq = mfc_slope * employees[1] + supply_2024_intercept  # MFC at L=120
...
plt.xlabel('Number of Employees')
plt.ylabel('Annual Wage ($)')
plt.title('Equilibrium in the Software Engineering Firm with Flat Labor Supply Curves')
plt.grid(True, alpha=0.3)
plt.legend(loc='lower left')
...
supply_slope = 1200  # Even steeper - $1200 increase per additional employee
...
# Adjust MFC intercept so that at 60 workers, MFC = $75,000
target_mfc_at_60 = 75000  # Target MFC value at 60 workers
mfc_intercept_adjustment = target_mfc_at_60 - mfc_slope * 60

# For 2023: Calculate shift needed relative to supply curve
mfc_2023_intercept = mfc_intercept_adjustment
y_mfc_2023 = mfc_slope * x_line + mfc_2023_intercept

# For 2024: Keep the same relationship (parallel shift)
mfc_shift_between_years = supply_2024_intercept - supply_2023_intercept
mfc_2024_intercept = mfc_2023_intercept + mfc_shift_between_years
...
![Share of Workers with Non-Compete Agreements by Education](/images/05_images/noncompetes2.png){#fig-noncompetes-education width=70%}

![Share of Workers with Non-Compete Agreements by Industry](/images/05_images/noncompetes4.png){#fig-noncompetes-industry width=70%}
...
```{python}
#| echo: false
#| label: fig-merger-earnings-impact
#| fig-cap: "Impact of Different Types of Mergers on Worker Earnings"

import matplotlib.pyplot as plt
import numpy as np

# Data from @arnold2025mergers
merger_types = ['Low-Impact\nMergers', 'Medium-Impact\nMergers', 'High-Impact\nMergers']
earnings_impact = [-0.5, -0.8, -3.1]  # Percent change in earnings

# Create colors - using blue color scheme
colors = ['#2E86AB', '#1E5F7A', '#1A4E6B']  # Light to dark blue

plt.figure(figsize=(7, 5))

# Create bar chart
bars = plt.bar(merger_types, earnings_impact, color=colors, alpha=0.8, 
               edgecolor='black', linewidth=1)

# Add value labels on top of bars
for i, (bar, value) in enumerate(zip(bars, earnings_impact)):
    plt.text(bar.get_x() + bar.get_width()/2, value - 0.15, 
             f'{value}%', ha='center', va='top', fontweight='bold', fontsize=11)

# Customize the plot
plt.ylabel('Change in Earnings (%)', fontsize=12, fontweight='bold')
plt.xlabel('Type of Merger', fontsize=12, fontweight='bold')
plt.title('Impact of Mergers on Worker Earnings by Merger Type', 
